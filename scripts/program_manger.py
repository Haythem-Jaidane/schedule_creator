...
class create_table:
    ...
    def date_system(self,str_date):
        ...
        self.date_tuple = (
            f"{self.start_date.day}/{self.start_date.month}",
            f"{str(self.end_date.day)}/{str(self.end_date.month)}",
        )
    ...
    def create_data(self):
        ...
        for i in range(self.start_hour,self.end_hour):
            row = [(f"{str(i)}h to {str(i + 1)}h", 1)]
            row.extend(["free",1] for _ in range(self.number))
    ...
    def web2pdf(self,to_say=""):
        ...
        pdf_object.cell(
            pdf_object.w,
            txt=f"From {self.date_tuple[0]} to {self.date_tuple[1]}",
            align="C",
        )
        pdf_object.ln(10)
        ...
        i=0
        history = [[0,True] for _ in range(self.number+1)]
        ...
        pdf_object.output(f"../uploads/{self.file_name}.pdf")